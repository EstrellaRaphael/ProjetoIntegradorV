...
function statusBadge(status: AlunoStatus) {
  const map: Record<AlunoStatus, string> = {
    ATIVO: 'badge badge-success',
    INATIVO: 'badge badge-neutral',
    TRANCADO: 'badge badge-warning',
  }
  const labels: Record<AlunoStatus, string> = { ATIVO: 'Ativo', INATIVO: 'Inativo', TRANCADO: 'Trancado' }
                  ...
                  {students.map((s) => (
                    <tr key={s.id} className="tr-row">
                      ...
                      <td className="td">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-full bg-primary-fixed flex items-center justify-center flex-shrink-0">
                            <span className="text-primary text-xs font-bold">{getInitials(s.nome_completo)}</span>
                          </div>
                      ...
                      <td className="td text-on-surface-variant">
                        {s.turma_atual_id ? s.turma_atual_id.slice(0, 8) + '…' : '—'}
                      </td>
                      <td className="td">{statusBadge(s.status)}</td>
                      <td className="td-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            className="p-1.5 rounded-lg hover:bg-error-container/50 transition-colors text-on-surface-variant hover:text-on-error-container"
                            title="Excluir"
